...
const NONE = { notifications: 0, reviews: 0, bounties: 0, wallet: 0 };
...
test("a notifications-changed frame refreshes notifications AND the review queue", () => {
  // notifyForReview fires when a PR joins the queue and again at the terminal
  // verdict — the two moments the queue must reflect. Routing this frame to
  // `reviews` is what lets the Agent page stop polling.
  ...
  timer.fire();
  assert.deepEqual(n, { notifications: 1, reviews: 1, bounties: 0, wallet: 0 });
});
...
test("a bounties-changed frame routes to the bounties topic only", () => {
  ...
  timer.fire();
  assert.deepEqual(n, { notifications: 0, reviews: 0, bounties: 1, wallet: 0 });
});
...
test("a notifications-read frame refreshes notifications only", () => {
  ...
  timer.fire();
  assert.deepEqual(n, { notifications: 1, reviews: 0, bounties: 0, wallet: 0 });
});
...
test("the reviews topic gets a longer window than the default", () => {
  // It replaces a 3s poll. At the 250ms default a burst could refetch ~12x more
  // often than the thing it replaced — and this screen has a documented
  // refetch-storm history. The window must stay comfortably under the old cadence.
  const timer = fakeTimer();
  const bus = createLiveBus({ setTimer: timer.setTimer, clearTimer: timer.clearTimer });
  counters(bus);
  bus.deliver(frame("notifications-changed")); // routes to notifications + reviews
  assert.equal(timer.ms, REVIEWS_DEBOUNCE_MS);
  assert.ok(REVIEWS_DEBOUNCE_MS > DEBOUNCE_MS, "and it must actually be longer");
});

test("a frame with no reviews topic keeps the fast default window", () => {
  const timer = fakeTimer();
  const bus = createLiveBus({ setTimer: timer.setTimer, clearTimer: timer.clearTimer });
  counters(bus);
  bus.deliver(frame("bounties-changed"));
...
test("subscribing to an unknown topic is reported loudly", () => {
  // Most screens in this app carry `// @ts-nocheck`, so a mistyped topic would
  // not be caught at build time — it would just silently never fire. Trade the
  // silence for a console error at subscribe time.
  const timer = fakeTimer();
  const bus = createLiveBus({ setTimer: timer.setTimer, clearTimer: timer.clearTimer });
  ...
  try {
    bus.subscribe("reviws" as Topic, () => {});
  } finally {
    console.error = realError;
  }
  assert.equal(errors.length, 1);
  assert.match(String(errors[0]), /unknown topic "reviws"/);
});
...
test("a malformed frame refreshes everything and reports the raw payload", () => {
  ...
  timer.fire();
  assert.deepEqual(n, { notifications: 1, reviews: 1, bounties: 1, wallet: 1 });
  bus.deliver(JSON.stringify({ no: "type" }));
  timer.fire();
  assert.deepEqual(n, { notifications: 2, reviews: 2, bounties: 2, wallet: 2 });
