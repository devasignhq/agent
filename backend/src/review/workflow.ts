...
// (policy + automation) and `actions` (GitHub Actions workflow dispatch) are
// ADVANCED (Pro/Max) — the API rejects advanced changes from free users (see
// advancedChanged); the UI locks them.
//
// Per-stage custom prompts (`prompts`) are DISABLED: normalizeWorkflow drops any
// inbound text and effectiveWorkflow ignores anything already stored, so every
// stage runs on its base system prompt (withMaintainerInstructions sees "").
...
// actions). The API uses this to refuse advanced changes from free
