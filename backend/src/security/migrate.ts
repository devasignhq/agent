...
// then capped by the needs_human confidence (legacy findings were never
// human-triaged or evidence-verified, and unconfirmed findings can't sit above
// medium). The first real audit run re-derives severity on matched
// fingerprints.
  ...
  for (const entry of db.table("repoIndex")) {
    for (const v of entry.vulnerabilities ?? []) {
      ...
      const row: SecurityFinding = {
        ...
        severity: capSeverityByConfidence(v.severity === "blocker" ? "high" : "medium", "needs_human"),
